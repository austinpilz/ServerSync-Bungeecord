...
public class BungeeIncoming implements Listener
{
    ...
    public void onPluginMessage(PluginMessageEvent ev) throws IOException 
    {
        ...
        else if (subChannel.equals(ServerSync.checkinSubChannel))
        {
        	...
        	{
        		Log.error("Checkin - error while attempting to read in message fields");
        	}
	        	...
	        	if (operation.equals("Checkin")) //Server is registering itself with the hub
	        	{
	        		if (!serverName.isEmpty())
	        		{
	        			if (!ServerSync.servers.containsKey(serverName))
	        			{
	        				...
	        				{
	        					Log.info(ServerSync.consolePrefix + "Added new server ["+serverName+"]");
	        				}
	        			}
	        			else
	        			{
	        		...
	        		{
	        			Log.error(ServerSync.consolePrefix + "Checkin requested, but no server name was supplied");
	        		}
	        	}
	        	else
	        	{
	        		//Unknown Operation
	        		Log.error(ServerSync.consolePrefix + "Unknown operation ("+operation+") supplied for check-in operation");
	        	}
        	}
        	else
        	{
        		//Version Mismatch
        		Log.error(ServerSync.consolePrefix + "Checkin - messaged received from client ("+serverName+"). Hub Version " + ServerSync.pluginVersion + " | Client Version " + clientVersion);
        	}
        }
        else
        {
        	//Unknown subchannel
        	Log.info(ServerSync.consolePrefix + "Received a message on an unknown subchannel ("+subChannel+")");
        }
    }
}
